...
*/
        pub async fn resolve_principal_permissions(
            &mut self,
            request: impl tonic::IntoRequest<super::ResolvePrincipalPermissionsRequest>,
        ) -> std::result::Result<
        ...
        > {
            ...
            let codec = tonic_prost::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static(
                "/pidgr.v1.AuthorizationService/ResolvePrincipalPermissions",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(
...
pub mod authorization_service_server {
    ...
    pub trait AuthorizationService: std::marker::Send + std::marker::Sync + 'static {
        /** Resolve the effective permissions for one (subject, org, principal type).
*/
        async fn resolve_principal_permissions(
            &self,
...
*/
        ...
        B::Error: Into<StdError> + std::marker::Send + 'static,
    {
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/pidgr.v1.AuthorizationService/ResolvePrincipalPermissions" => {
                    #[allow(non_camel_case_types)]
                    struct ResolvePrincipalPermissionsSvc<T: AuthorizationService>(
                        pub Arc<T>,
                    );
                    impl<
                        T: AuthorizationService,
                    > tonic::server::UnaryService<
                        super::ResolvePrincipalPermissionsRequest,
                    > for ResolvePrincipalPermissionsSvc<T> {
                        type Response = super::ResolvePrincipalPermissionsResponse;
                        type Future = BoxFuture<
                            tonic::Response<Self::Response>,
                            tonic::Status,
                        >;
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as AuthorizationService>::resolve_principal_permissions(
                                        &inner,
                                        request,
                                    )
                                    .await
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "pidgr.v1.AuthorizationService";
